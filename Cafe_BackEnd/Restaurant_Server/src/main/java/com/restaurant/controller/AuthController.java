...
//    controller -> service -> dto(if needed) -> entity (and vice versa)
//    we have created dto package for dto type classes for serialization and deserializatio purpose.
//    we had to tell our spring application that without jwt token also we can able to access this signup and login api.
//    so for that we have implemented securityfilterchain in our webconfiguration file. 
    @PostMapping("/signup")
    public ResponseEntity<?> signupUser(@RequestBody(required = true) SignupRequest signupRequest) throws Exception { //first created object reference of dto type class

        if (authService.hasUserWithEmail(signupRequest.getEmail())) // calls method present in authservice interface ->  authserviceimpl.
            return new ResponseEntity<>("User already exists", HttpStatus.NOT_ACCEPTABLE); //return code is 406

        UserDto createdUser = authService.createUser(signupRequest);
        if (createdUser == null)
            return new ResponseEntity<>("User not created, come again later", HttpStatus.NOT_ACCEPTABLE);

        return new ResponseEntity<>(createdUser, HttpStatus.OK); //return code 200
    }
