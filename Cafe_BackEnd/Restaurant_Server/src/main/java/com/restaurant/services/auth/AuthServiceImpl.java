...
public class AuthServiceImpl implements AuthService {
    ...
    private final UserRepo userRepo;  //declaring userRepo of type UserRepo to create object of UserRepo type.
    ...
    public void createAdminAccount() {
        ...
        if (null == adminAccount) {
            ...
            user.setPassword(new BCryptPasswordEncoder().encode("admin")); //bcryptpasswordencoder is used to encode password so that unauthorized user cant access the password from the database.
            userRepo.save(user);
        }
    }

//	this injects data to the entity file.
    @Transactional
    public UserDto createUser(SignupRequest signupRequest) throws Exception {
        ...
        User createdUser = userRepo.save(user); //saving data to the repo of the user entity file.
        UserDto createdUserDto = new UserDto(); //creating object of dto type class and insert id to it and return that for checking in controller.
    ...
    public Boolean hasUserWithEmail(String email) {
        return userRepo.findFirstByEmail(email).isPresent(); //returning to the authcontroller if user exists by checking email
    }
